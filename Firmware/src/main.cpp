/**
 *  5ch MAX31855K thermocouple amp to CAN firmware
 *
...
// Ensure all CS pins are deasserted
void setupGPIOs() {
  for (int i = 0; i < NUM_CHANNELS; i++) {
...
// Set CAN bitrate and setup messages
void setupCAN() {
...
// Check if all LEDs are alive. Also be obnoxious.
void blinkLEDs() {
  pwrLED.write(1);
  wait_ms(250);
  txLED.write(1);
